...
1. **原始数据层（Raw Sources）**：不可变的原始文件（文章、论文、笔记、图片），LLM 只读不写
2. **编译输出层（The Wiki）**：LLM 全权维护的 Markdown 文件集合，包括摘要、概念、实体、综合判断等
3. **控制协议层（The Schema）**：定义目录结构、页面格式、操作流程的配置文档（如 `CLAUDE.md`）

### 三大操作

| 操作 | 功能 | 人类角色 | LLM 角色 |
|------|------|----------|----------|
| 摄入（Ingest） | 摄入新材料，更新知识体系 | 确认重点、引导方向 | 提取要点、建立链接、更新页面 |
| 查询（Query） | 提问并获得带引用的答案 | 提出好问题 | 搜索、综合、生成答案 |
| 检查（Lint） | 健康检查 | 审阅报告 | 检测矛盾、发现孤岛、标记过时内容 |
